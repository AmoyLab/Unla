...
func (db *MySQL) GetToolStatus(ctx context.Context, tenant, server, toolName string) (*MCPToolStatus, error) {
...
func (db *MySQL) SetToolStatus(ctx context.Context, tenant, server, toolName string, enabled bool) error {
	var status MCPToolStatus
	err := db.db.WithContext(ctx).
		Where("tenant = ? AND server = ? AND tool_name = ?", tenant, server, toolName).
		First(&status).Error
...
func (db *MySQL) GetToolStatuses(ctx context.Context, tenant, server string) ([]*MCPToolStatus, error) {
